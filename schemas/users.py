"""User models and schemas (Postgres/UUID)."""
    ...
    def validate_metadata(
        ...
        for provider, metadata in v.items():
            if provider == "byo-openai":
                try:
                    validated_metadata["byo-openai"] = BYOOpenAIMetadata(**metadata)
                except Exception:
                    raise ValueError(
                ...
                try:
                    validated_metadata["byo-azure-openai"] = BYOAzureOpenAIMetadata(**metadata)
                except Exception:
                    raise ValueError(
...
class UserDocumentModel(BaseDocument):
    ...
    注意：用户表一般没有 created_by 概念（自创建），这里沿用 BaseDocument 的时间戳，
    created_by 可置 None（与现有实现一致）。
    """
...
class SetProvidersDetailsResponse(BaseResponse, ProviderConfig):
    """Response body of the set providers details endpoint."""
    pass


class GetProvidersDetailsResponse(BaseResponse, ProviderConfig):
    """Response body of the get providers details endpoint."""
    pass
...
class DeleteUserResponse(BaseResponse):
    """Response body of the delete user endpoint."""
    pass
